...
def main():
    ...
    absolute_folder_path = Path(folder_images_path).resolve()
    absolute_folder_path = str(absolute_folder_path).replace("\\", "/")

    _img_names = convert_data_to_Labeltxt(df, folder_images_path, _ImageName_Column, _PositionBBoxName_Column, _OCRName_Column)
    convert_data_to_fileStatetxt(absolute_folder_path, _img_names)
